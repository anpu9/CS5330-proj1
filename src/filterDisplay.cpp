...
void processMagnitude(Mat& original) {
    Mat x;
    Mat y;
    Mat mag;
    sobelX3x3(original, x);
    sobelY3x3(original, y);
    magnitude(x, y, mag);
    imshow("Magnitude", mag);

    int key = waitKey(0);
    if (key == 's') { // Save Vignetting image
        imwrite("../outputs/photoshot-maginitude2.png", mag);
        cout << "Saved vignetting image." << endl;
    }
}
...
int main() {
    string image_path = "../outputs/silk.jpeg";
